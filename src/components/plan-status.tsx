...
export function PlanStatus() {
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .single()
      .then(
        ({ data }) => setPlan((data?.plan as Plan) ?? "starter"),
        () => setPlan("starter"),
      );
  }, []);
    ...
    <div className="mt-6 rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      {isPaid ? (
        ...
        <button
          ...
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-mint py-2 text-xs font-bold text-white shadow-[0_6px_16px_rgba(24,184,143,.3)] transition hover:-translate-y-0.5"
        >
          Fazer upgrade → Pro
        </button>
      )}
